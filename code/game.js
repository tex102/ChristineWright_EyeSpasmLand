...
// Title Screen
function LastScreen(pos, ch) {
  this.pos = pos;
  this.size = new Vector(10,10);	
}
LastScreen.prototype.type = "lastScreen";


// Title Screen
function TitleScreen(pos, ch) {
  this.pos = pos;
  this.size = new Vector(10,10);	
}
TitleScreen.prototype.type = "titleScreen";
...
Globe.prototype.type = "globe";
//Big W
function BigW(pos, ch){
    this.pos = pos;
	this.size = new Vector(2,2);
	this.speed = new Vector(2,0);	
}
BigW.prototype.type = "bigW";
...
SmE.prototype.type = "smE";

//Big E
function BigE(pos, ch){
	this.pos = pos;
	this.size = new Vector(1,1);
	this.speed = new Vector(0,10);
}
BigE.prototype.type = "bigE";
...
SmP.prototype.type = "smP";

//Big P
function BigP(pos, ch){
	this.pos = pos;
	this.size = new Vector(2,2);
	this.speed = new Vector(0,2);
}
BigP.prototype.type = "bigP";
...
TitleScreen.prototype.act = function(step, level){
  
};
LastScreen.prototype.act = function(step, level){
  
};

SmE.prototype.act = function(step, level){
...
BigE.prototype.act = function(step, level){
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
//pink block cluster
SmP.prototype.act = function(step, level){
	  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
//orange block cluster
BigP.prototype.act = function(step, level){
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
SmW.prototype.act = function(step, level){
	  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
BigW.prototype.act = function(step, level){
	  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
Globe.prototype.act = function(step) {
...
var gravity = 30;
var jumpSpeed = 17;
var jump = new Audio("audio/sfx/jump.mp3");
	jump.volume = 0.5;
...
var hasPlayed = 0;
if(keys.up){
	    document.querySelector("jump.mp3");
		jump.play();
...
Level.prototype.playerTouched = function(type, actor) {
	...
	if (type == "smP" && this.status == null){
		var shocked = new Audio("audio/sfx/shock.mp3");
		shocked.volume = 0.4;
		  this.status = "lost";	
		  document.querySelector("shock.mp3");
		  shocked.play();
          this.finishDelay = 2;	
	};
	
	if (type == "bigP"  && this.status == null){
		var shocked = new Audio("audio/sfx/shock.mp3");
		shocked.volume = 0.4;
		  this.status = "lost";	
		  document.querySelector("shock.mp3");
	...
	var coinSound = new Audio("audio/sfx/coinSound.mp3");
		coinSound.volume = 0.5;
	var lavaSound = new Audio("audio/sfx/flames.mp3");
  // if the player touches lava and the player hasn't won
  // Player loses
  if (type == "lava" && this.status == null) {
    this.status = "lost";
	document.querySelector("flames.mp3");
		lavaSound.play();
	
    this.finishDelay = 2;
  } else if (type == "coin") {
	document.querySelector("coinSound.mp3");
		coinSound.play();
...
function runGame(plans, Display) {
	var winSound = new Audio("audio/sfx/win.mp3");
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "lost")
        ...
        console.log("You win!");
		document.querySelector("win.mp3");
		winSound.play();
    });
  }
  startLevel(0);
